...
#define dpin_out 6  // Added to match UneditedCode 
...
void setup() {
  ...
  pinMode(dpin_in, INPUT);
  pinMode(dpin_out, OUTPUT);    // Initialize output trigger 
  digitalWrite(dpin_out, LOW);   // Ensure it starts LOW 
}

void loop() {
  // Check if lock is engaged (match UneditedCode logic) 
  if (digitalRead(dpin_in) == HIGH) {
    digitalWrite(dpin_out, HIGH); // Signal the start of the sweep 
    unsigned long start_time = micros();
    
    // Polling for signal peaks while the lock is active
    int value1 = analogRead(pin_input1);
    ...
    // Reset the trigger output after the cycle 
    digitalWrite(dpin_out, LOW); 
  } else {
    // Optional: match the "Lock disengaged" feedback from UneditedCode 
